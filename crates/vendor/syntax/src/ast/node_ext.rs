...
impl ast::Meta {
    ...
    pub fn as_simple_call(&self) -> Option<(SmolStr, ast::TokenTree)> {
        let ast::Meta::TokenTreeMeta(meta) = self else {
            return None;
        };
        Some((
            meta.path()?.as_single_name_ref()?.text().into(),
            meta.token_tree()?,
        ))
    }

    pub fn as_simple_path(&self) -> Option<ast::Path> {
        let ast::Meta::PathMeta(meta) = self else {
            return None;
        };
        meta.path()
    }
    ...
    pub fn skip_cfg_attrs(self) -> SmallVec<[ast::Meta; 1]> {
        match self {
            ast::Meta::CfgAttrMeta(meta) => meta
                .metas()
                .flat_map(|meta| meta.skip_cfg_attrs())
                .collect(),
            _ => smallvec![self],
        }
    }
...
impl ast::TypeBound {
    pub fn kind(&self) -> Option<TypeBoundKind> {
        if let Some(path_type) = support::children(self.syntax()).next() {
            ...
            let Some(ast::Type::PathType(path_type)) = for_binder.ty() else {
                return None;
            };
