//! Adapted from rust-analyzer (2026-09-21).
//!
...
//! The encoding is documented in the [`TokenTreesSlice::decode()`] function (which decodes one [`TokenTree`]).
...
use crate::{
    ...
    tt::{
        DelimSpan, Delimiter, DelimiterKind, Ident, IdentIsRaw, Leaf, LitKind, Literal, Punct,
        Spacing, Subtree, SubtreeView, TokenTree, TokenTreesView, TtIter, symbol::Symbol,
    },
};
    ...
    unsafe fn finish(mut self, writer_finish: *mut u8) -> Box<[u8]> {
        ...
        unsafe {
            ...
            buffer
                .as_mut_ptr()
                .cast::<u8>()
                .copy_from_nonoverlapping(writer_finish, bytes_len);
            buffer.assume_init()
        }
    }
}
...
impl<'a> BufferWriter<'a> {
    ...
    unsafe fn write<T: Encodable>(&mut self, value: T) {
        #[cfg(all(debug_assertions, not(miri)))]
        {
            let write_at = self
                .buffer
                .split_off(self.buffer.len() - size_of::<T>()..)
                .unwrap();
...
unsafe fn encode_extended_span(
        ...
        && !force_heavy_encoding
    {
        ...
        assert!(
            span_parts_index <= n_bits_mask(24),
            "too big `span_parts_index`"
        );
    ...
    unsafe {
        match tt {
            ...
            TokenTree::Leaf(Leaf::Literal(Literal {
                ...
                if matches!(
                    kind,
                    LitKind::Str | LitKind::StrRaw(0 | 1) | LitKind::Integer
    ...
    unsafe {
        ...
        for (index, tt) in token_trees.rev() {
            ptr = encode(
                ...
                tt,
                &span_parts_map,
                &mut byte_size_after[index..],
                &symbols_map,
            );
        }
        ptr = encode_top_subtree(
            ptr,
            top_subtree,
            (span_parts.len() - 2) as u32,
            &byte_size_after,
        );
...
impl<'a> TokenTreesSlice<'a> {
    ...
    unsafe fn decode_span(
        ...
        if !extends_to_next {
            ...
            unsafe { Self::decode_extended_span(ptr, span_parts_index, len, offset) }
        }
    }
    ...
    unsafe fn decode_extended_span(
        ...
        unsafe {
            ...
            if extends_to_next {
                ...
                let rest_span_parts_index = (u64 & Self::n_bits_mask_u64(24)) as u32;
                span_parts_index |= rest_span_parts_index << 4;
                let rest_len = ((u64 >> 24) & Self::n_bits_mask_u64(24)) as u32;
                len |= rest_len << 8;
    ...
    unsafe fn decode_symbol(
        ...
        if extends_to_next {
            ...
            let rest_bytes = if second_byte & 0b1 == 0b1 {
                u32::from(unsafe { ptr.read::<u16>() })
            } else {
                0
            };
            second_byte >>= 1;
    ...
    unsafe fn decode(
        ...
        unsafe {
            ...
            let span;
            (ptr, span) = Self::decode_span(ptr, span_and_extra);
            ...
            if span_and_extra & 0b1 == 0b1 {
                ...
                let symbol;
                (ptr, symbol) = Self::decode_symbol(ptr, symbol_first_byte, symbols);
            ...
            let result = match control_byte & 0b111 {
                0b000 => {
                    ...
                    let kind =
                        transmute::<u8, DelimiterKind>((control_byte_extra_data & 0b11) as u8);
                ...
                0b101 => {
                    ...
                    let close_span;
                    (ptr, close_span) = Self::decode_span(ptr, close_span_start);
                    let close_span =
                ...
                0b011 | 0b100 => {
                    ...
                    let text_and_suffix;
                    (ptr, text_and_suffix) =
                        Self::decode_symbol(ptr, text_and_suffix_first_byte, symbols);
    ...
    fn new(top_subtree: &'a TopSubtree) -> Self {
        ...
        Self {
            ...
            span_parts: &top_subtree.span_parts,
            symbols: &top_subtree.symbols,
        }
    }
    ...
    pub(crate) fn advance(&mut self) -> Option<TokenTree> {
        ...
        let (new_current, token_tree, _children_byte_len) =
            unsafe { Self::decode(self.current, self.span_parts, self.symbols) };
...
impl TopSubtree {
    ...
    fn compute_span_frequencies_and_symbols(
        ...
        FxHashMap<CompressedSpanPart, usize>,
        ...
        for tt in tts {
            match tt {
                TokenTree::Leaf(leaf) => {
                    ...
                    *span_frequencies
                        .entry(CompressedSpanPart::from_span(leaf.span()))
                        .or_insert(0) += 1;
                }
                TokenTree::Subtree(subtree) => {
                    *span_frequencies
    ...
    pub fn from_serialized(tts: Vec<TokenTree>) -> Self {
        let (span_frequencies, symbols) = Self::compute_span_frequencies_and_symbols(&tts[1..]); // Do not include the top subtree.
    ...
    pub fn set_top_subtree_delimiter_kind(&mut self, kind: DelimiterKind) {
        Self::change_root_delimiter(&mut self.buffer, kind);
    }
    ...
    pub fn set_token(&mut self, idx: usize, leaf: Leaf) {
        ...
        assert_matches!(
            tts[idx],
            TokenTree::Leaf(_),
            "cannot change a subtree to a leaf"
        );
        tts[idx] = leaf.into();
    ...
    pub fn change_every_ast_id(
        &mut self,
        mut callback: impl FnMut(&mut crate::span::ErasedFileAstId),
    ) {
...
// The byte decoder trusts its in-memory encoding. Persist logical tokens instead, then validate
// their nesting and literal boundaries before rebuilding that encoding during a cache load.
// Writing directly from the iterator avoids materializing another expanded tree while offloading.
unsafe impl<C: wincode::config::Config> wincode::SchemaWrite<C> for TopSubtree {
    type Src = Self;
    const TYPE_META: wincode::TypeMeta = wincode::TypeMeta::Dynamic;

    fn size_of(src: &Self) -> wincode::WriteResult<usize> {
        use wincode::len::SeqLen as _;
        let mut size = C::LengthEncoding::write_bytes_needed(src.len)?;
        for token in src.as_token_trees().iter_flat_tokens() {
            size += <TokenTree as wincode::SchemaWrite<C>>::size_of(&token)?;
        }
        Ok(size)
    }
    ...
    fn write(mut writer: impl wincode::io::Writer, src: &Self) -> wincode::WriteResult<()> {
        ...
        for token in src.as_token_trees().iter_flat_tokens() {
            <TokenTree as wincode::SchemaWrite<C>>::write(writer.by_ref(), &token)?;
        }
        Ok(())
    }
}

unsafe impl<'de, C: wincode::config::Config> wincode::SchemaRead<'de, C> for TopSubtree {
    type Dst = Self;
    const TYPE_META: wincode::TypeMeta = wincode::TypeMeta::Dynamic;

    fn read(
        reader: impl wincode::io::Reader<'de>,
        dst: &mut std::mem::MaybeUninit<Self>,
    ) -> wincode::ReadResult<()> {
        use wincode::ReadError;
        let tokens = <Vec<TokenTree> as wincode::SchemaRead<'de, C>>::get(reader)?;
        let Some(TokenTree::Subtree(root)) = tokens.first() else {
            return Err(ReadError::InvalidValue("token tree must have a root"));
        };
        if root.usize_len() != tokens.len() - 1 {
            return Err(ReadError::InvalidValue("invalid token-tree root length"));
        }

        // Each child must end inside its parent. This also bounds all offsets used by the encoder
        // when it computes subtree byte lengths backwards through the flattened token stream.
        let mut ends = vec![tokens.len()];
        for (index, token) in tokens.iter().enumerate().skip(1) {
            while ends.last() == Some(&index) {
                ends.pop();
            }
            match token {
                TokenTree::Subtree(subtree) => {
                    let end = index
                        .checked_add(1 + subtree.usize_len())
                        .filter(|end| *end <= *ends.last().expect("root covers every token"))
                        .ok_or(ReadError::InvalidValue("invalid token-tree subtree length"))?;
                    ends.push(end);
                }
                TokenTree::Leaf(Leaf::Literal(literal)) => {
                    let text = literal.text_and_suffix.as_str();
                    let valid_suffix = text
                        .len()
                        .checked_sub(usize::from(literal.suffix_len))
                        .is_some_and(|start| text.is_char_boundary(start));
                    if !valid_suffix {
                        return Err(ReadError::InvalidValue("invalid token-tree literal suffix"));
                    }
                }
                TokenTree::Leaf(_) => {}
            }
        }
        dst.write(Self::from_serialized(tokens));
        Ok(())
    }
}

rg_std::memsize::impl_memory_size_leaf!(CompressedSpanPart);
rg_std::memsize::impl_memory_size_children!(TopSubtree => buffer, span_parts, symbols);
...
impl TopSubtreeBuilder {
    ...
    pub fn new(top_delimiter: Delimiter) -> Self {
        ...
        result.token_trees.push(TokenTree::Subtree(Subtree {
    ...
    pub fn extend_with_tt(&mut self, tt: TokenTreesView<'_>) {
        tt.iter_flat_tokens()
            .for_each(|tt| self.push_token_tree(tt));
    }
    ...
    pub fn restore(&mut self, restore_point: SubtreeBuilderRestorePoint) {
        ...
        self.unclosed_subtree_indices
            .truncate(restore_point.unclosed_subtree_indices_len);
