...
impl SpanStorage for SpanStorage64 {
    ...
    fn new(text_range: TextRange, span_parts_index: usize) -> Self {
        ...
        Self {
            offset,
            len_and_parts: (len << Self::SPAN_PARTS_BIT) | span_parts_index,
        }
    }
...
impl<S: SpanStorage> TokenTree<S> {
    #[inline]
    pub(crate) fn to_api(&self, span_parts: &[CompressedSpanPart]) -> crate::tt::TokenTree {
        match self {
            TokenTree::Literal {
                text_and_suffix,
                span,
                kind,
                suffix_len,
            } => crate::tt::TokenTree::Leaf(crate::tt::Leaf::Literal(crate::tt::Literal {
            ...
            TokenTree::Punct {
                ...
                spacing,
                span,
            } => crate::tt::TokenTree::Leaf(crate::tt::Leaf::Punct(crate::tt::Punct {
                char: *char,
                spacing: *spacing,
                span: span.span(span_parts),
            })),
            TokenTree::Ident { sym, span, is_raw } => {
                crate::tt::TokenTree::Leaf(crate::tt::Leaf::Ident(crate::tt::Ident {
                    sym: sym.clone(),
                    span: span.span(span_parts),
                    is_raw: *is_raw,
                }))
            ...
            TokenTree::Subtree {
                ...
                open_span,
                close_span,
            } => crate::tt::TokenTree::Subtree(crate::tt::Subtree {
    ...
    fn convert<U: From<S>>(self) -> TokenTree<U> {
        match self {
            ...
            TokenTree::Ident { sym, span, is_raw } => TokenTree::Ident {
                sym,
                span: span.into(),
                is_raw,
            },
...
pub struct TopSubtreeBuilder {
    ...
    span_parts: Vec<CompressedSpanPart>,
...
impl TopSubtreeBuilder {
    pub fn new(top_delimiter: crate::tt::Delimiter) -> Self {
        let mut result = Self {
        ...
        fn push_first<S: SpanStorage>(
            tt: &mut Vec<TokenTree<S>>,
            top_delimiter: crate::tt::Delimiter,
        ) {
    ...
    fn span_part_index(&mut self, part: CompressedSpanPart) -> usize {
        let index = self
            ...
            .unwrap_or_else(|| {
                self.span_parts.push(part);
                self.span_parts.len() - 1
            });
        index + RESERVED_SPAN_PARTS_LEN
    }
    ...
    fn push_subtree(&mut self, subtree: crate::tt::Subtree) {
    ...
    fn push_literal(&mut self, leaf: crate::tt::Literal) {
    ...
    fn push_punct(&mut self, leaf: crate::tt::Punct) {
        let span_parts_index = self.span_part_index(CompressedSpanPart::from_span(&leaf.span));
        let range = leaf.span.range;
        self.ensure_can_hold(range, span_parts_index);
        fn do_it<S: SpanStorage>(
    ...
    fn push_ident(&mut self, leaf: crate::tt::Ident) {
        let span_parts_index = self.span_part_index(CompressedSpanPart::from_span(&leaf.span));
        let range = leaf.span.range;
        self.ensure_can_hold(range, span_parts_index);
        fn do_it<S: SpanStorage>(
    ...
    pub fn push(&mut self, leaf: crate::tt::Leaf) {
        match leaf {
            crate::tt::Leaf::Literal(leaf) => self.push_literal(leaf),
            crate::tt::Leaf::Punct(leaf) => self.push_punct(leaf),
            crate::tt::Leaf::Ident(leaf) => self.push_ident(leaf),
        }
    }

    fn push_token_tree(&mut self, tt: crate::tt::TokenTree) {
        match tt {
            crate::tt::TokenTree::Leaf(leaf) => self.push(leaf),
            crate::tt::TokenTree::Subtree(subtree) => self.push_subtree(subtree),
        }
    }

    pub fn extend(&mut self, leaves: impl IntoIterator<Item = crate::tt::Leaf>) {
