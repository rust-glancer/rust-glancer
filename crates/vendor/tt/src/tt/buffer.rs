...
use crate::tt::{Leaf, Subtree, TokenTree, TokenTreesView, storage::TokenTreesSlice};
...
impl<'a> Cursor<'a> {
    ...
    pub(crate) fn remaining(&self) -> TokenTreesView<'a> {
        TokenTreesView {
            slice: self.buffer_before_current,
            len: self.len - self.advances_count,
        }
    }

    pub fn end(&mut self) -> Subtree {
        let (last_subtree_idx, last_subtree) = self
            .last_subtree()
            .expect("called `Cursor::end()` without an open subtree");
    ...
    fn advance(&mut self) {
        ...
        self.next = if self.advances_count < self.len {
            self.buffer_after_current.advance()
        } else {
            None
        };
    }
    ...
    pub fn peek_two_leaves(&self) -> Option<[Leaf; 2]> {
        ...
        let next_next = if self.advances_count + 1 < self.len {
            buffer.advance()
        } else {
            None
        };
