...
use crate::{
    ...
    proto::{references as references_proto, rename as rename_proto},
};
...
impl QueryRunner<'_> {
    ...
    /// Plan coverage before mutating the saved project for query-time materialization.
    ///
    /// Planning and the final query cannot share a snapshot: materialization may replace package
    /// payloads in the saved project. Callers therefore keep only owned plans across this boundary
    /// and resolve the cursor again afterward.
    ...
    pub(crate) fn references(
        ...
        let search_plans = self
            .reference_search_plans_for_position(&path, position, dirty.as_ref())
    ...
    pub(crate) fn document_highlight(
        ...
        let started = Instant::now();
        self.ensure_path("document_highlight", &path)
