...
//!
//! `ReferenceQuery` describes where the resolver should look: whole crates, selected files, or one
//! file. For example, LSP "find references" usually scans a project-chosen crate list, while a
...
impl<'a> ReferenceQuery<'a> {
    ...
    pub fn find_references(search_crates: &'a [CrateRef], include_declarations: bool) -> Self {
        ...
        Self {
            search_scope: ReferenceSearchScope::Crates(search_crates),
            declaration_policy,
        }
    }
    ...
    /// Returns a query scoped to one file inside one crate.
    pub fn file_scoped(crate_ref: CrateRef, file_id: FileId) -> Self {
        Self {
            search_scope: ReferenceSearchScope::File { crate_ref, file_id },
    ...
    pub(super) fn accepts_declaration(self, crate_ref: CrateRef, file_id: FileId) -> bool {
        match self.declaration_policy {
            ...
            ReferenceDeclarationPolicy::IncludeInSearchScope => match self.search_scope {
                ReferenceSearchScope::Crates(crates) => crates.contains(&crate_ref),
                ReferenceSearchScope::Files(files) => files
                    .iter()
                    .any(|file| file.crate_ref == crate_ref && file.file_id == file_id),
                ReferenceSearchScope::File {
                    crate_ref: selected_crate,
                    file_id: selected_file_id,
                } => selected_crate == crate_ref && selected_file_id == file_id,
            },
        }
    }
    ...
    pub(super) fn accepts_scan_target(self, scan: ReferenceScanTarget) -> bool {
        match self.search_scope {
            ReferenceSearchScope::Crates(crates) => crates.contains(&scan.crate_ref),
            ReferenceSearchScope::Files(files) => scan.file_id.is_some_and(|file_id| {
                files
                    .iter()
                    .any(|file| file.crate_ref == scan.crate_ref && file.file_id == file_id)
            }),
            ReferenceSearchScope::File { crate_ref, file_id } => {
                scan.crate_ref == crate_ref && scan.file_id == Some(file_id)
            }
        }
    }
}
