...
impl<'analysis, 'db, 'source> ImportItemCodeActionProvider<'analysis, 'db, 'source> {
    ...
    pub(super) fn code_actions(
        ...
        // 1. Select one complete source token. The first segment of `HashMap::new` has a
        // syntax-defined type namespace because every non-final path segment resolves there. Check
        // that shape before completion classification: a rebuilt body can also describe the same
        // token as an unqualified value-expression site, which is not the namespace Rust uses for
        // this segment. Standalone names still get their type/value namespace from completion.
        ...
        let qualified_root_context = ImportContext::for_qualified_root(
        ...
        let (import_context, lookup_name, definition_offset) =
            if let Some(import_context) = qualified_root_context {
