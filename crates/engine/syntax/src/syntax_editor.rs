...
impl SyntaxEditor {
    ...
    pub fn add_annotation(&self, element: impl Element, annotation: SyntaxAnnotation) {
        self.annotations
            .borrow_mut()
            .push((element.syntax_element(), annotation))
    }

    pub fn add_annotation_all(&self, elements: Vec<impl Element>, annotation: SyntaxAnnotation) {
        self.annotations.borrow_mut().extend(
            elements
                .into_iter()
                .map(|e| e.syntax_element())
                .zip(iter::repeat(annotation)),
        );
    }
    ...
    pub fn merge(&self, other: SyntaxEditor) {
        ...
        self.changes
            .borrow_mut()
            .append(&mut other.changes.into_inner());
        if let Some(mut m) = self.make.mappings() {
            m.merge(other.make.take());
        }
        self.annotations
            .borrow_mut()
            .append(&mut other.annotations.into_inner());
    }
    ...
    pub fn insert_all(&self, position: Position, elements: Vec<SyntaxElement>) {
        ...
        self.changes
            .borrow_mut()
            .push(Change::InsertAll(position, elements))
    }
    ...
    pub fn delete(&self, element: impl Element) {
        ...
        self.changes
            .borrow_mut()
            .push(Change::Replace(element.syntax_element(), None));
    }
    ...
    pub fn replace_all(&self, range: RangeInclusive<SyntaxElement>, new: Vec<SyntaxElement>) {
        ...
        self.changes
            .borrow_mut()
            .push(Change::ReplaceAll(range, new))
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Change::InsertAll(position, vec) => {
                ...
                parent_str.insert_str(
                    target_range.into(),
                    &format!("\x1b[42m{insertion}\x1b[0m\x1b[K"),
                );
                f.write_str(&parent_str)
            }
            ...
            Change::ReplaceAll(range, vec) => {
                ...
                write!(
                    f,
                    "{pre_str}\x1b[41m{old_str}\x1b[42m{new}\x1b[0m\x1b[K{post_str}"
                )
            }
        }
    }
...
mod tests {
    ...
    #[test]
    #[ignore = "rg_rowan bump arena fork is read-only; mutable syntax editing is unsupported"]
    fn basic_usage() {
