...
pub(super) fn apply_edits(editor: SyntaxEditor) -> SyntaxEdit {
    ...
    let mut get_node_depth = |node: SyntaxNode| {
        *node_depths
            .entry(node)
            .or_insert_with_key(|node| node.ancestors().count())
    };
    ...
    for (change_index, change) in changes.iter().enumerate() {
        ...
        if let Some(index) = changed_ancestors.iter().rev().position(|ancestor| {
            ancestor
                .affected_range()
                .contains_range(change.target_range())
        }) {
    ...
    for change in changes {
        match change {
            ...
            Change::Replace(SyntaxElement::Node(target), Some(new_target)) if target == root => {
                root = new_target
                    .into_node()
                    .expect("root node replacement should be a node");
            }
            Change::Replace(target, Some(new_target)) => {
...
enum ChangedAncestorKind {
    ...
    Range {
        _changed_elements: RangeInclusive<SyntaxElement>,
        _in_parent: SyntaxNode,
    },
}
...
impl ChangedAncestor {
    ...
    fn affected_range(&self) -> TextRange {
        match &self.kind {
            ...
            ChangedAncestorKind::Range {
                _changed_elements: changed_nodes,
                _in_parent: _,
            } => TextRange::new(
