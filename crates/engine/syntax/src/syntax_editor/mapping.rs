...
impl SyntaxMapping {
    ...
    pub fn upmap_child_element(
        ...
        match child {
            SyntaxElement::Node(node) => self
                .upmap_child(node, input_ancestor, output_ancestor)
            ...
            SyntaxElement::Token(token) => {
                ...
                debug_assert!(
                    element
                        .as_token()
                        .is_some_and(|it| it.kind() == token.kind()),
    ...
    pub fn upmap_child(
        ...
        debug_assert!(
            child == input_ancestor
                || child
                    .ancestors()
                    .any(|ancestor| &ancestor == input_ancestor)
        );
        ...
        let to_map_down = to_output_ancestor
            .into_iter()
            .rev()
            .chain(to_first_upmap.into_iter().rev());
    ...
    fn upmap_to_ancestor(
        ...
        let mut current = self
            ...
            .unwrap_or_else(|| input_ancestor.clone());
    ...
    pub fn upmap_node(
        ...
        let input_ancestor = input
            .ancestors()
            .find(|ancestor| self.upmap_node_single(ancestor).is_some());
    ...
    pub fn merge(&mut self, mut other: SyntaxMapping) {
        ...
        self.node_mappings
            .extend(other.node_mappings.into_iter().map(|(node, entry)| {
                (
                    node,
                    MappingEntry {
                        parent: entry.parent + remap_base,
                        ..entry
                    },
                )
            }));
    ...
    pub fn add_mapping(&mut self, syntax_mapping: SyntaxMappingBuilder) {
        ...
        let node_entries = node_mappings.into_iter().map(|(node, slot)| {
            (
                node,
                MappingEntry {
                    parent: parent_entry,
