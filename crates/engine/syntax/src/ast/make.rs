...
pub fn item_list(body: Option<Vec<ast::Item>>) -> ast::ItemList {
    ...
    ast_from_text(&format!(
        "mod C {{{body_newline}{body_indent}{body}{body_newline}}}"
    ))
}
...
pub fn assoc_item_list(body: Option<Vec<ast::AssocItem>>) -> ast::AssocItemList {
    ...
    let body_indent = if is_break_braces {
        "    ".to_owned()
    } else {
        String::new()
    };
    ...
    ast_from_text(&format!(
        "impl C for D {{{body_newline}{body_indent}{body}{body_newline}}}"
    ))
}
...
pub fn impl_(
    ...
    let attrs = attrs.into_iter().fold(String::new(), |mut acc, attr| {
        format_to_acc!(acc, "{}\n", attr)
    });
    ...
    let body_newline = if where_clause.is_some() && body.is_none() {
        "\n".to_owned()
    } else {
        String::new()
    };
    ...
    ast_from_text(&format!(
        "{attrs}impl{gen_params} {path_type}{gen_args}{where_clause}{body}"
    ))
}
...
pub fn impl_trait(
    ...
    let trait_gen_args = trait_gen_args
        .map(|args| args.to_string())
        .unwrap_or_default();
    let type_gen_args = type_gen_args
        .map(|args| args.to_string())
        .unwrap_or_default();
...
pub fn match_arm(pat: ast::Pat, guard: Option<ast::MatchGuard>, expr: ast::Expr) -> ast::MatchArm {
    ...
    let ws = guard
        .as_ref()
        .filter(|_| expr.is_block_like())
        .map_or(" ", block_whitespace);
    return match guard {
...
pub fn match_arm_list(arms: impl IntoIterator<Item = ast::MatchArm>) -> ast::MatchArmList {
    let arms_str = arms.into_iter().fold(String::new(), |mut acc, arm| {
        ...
        let comma = if needs_comma && arm.comma_token().is_none() {
            ","
        } else {
            ""
        };
...
pub fn item_static(
    ...
    ast_from_text(&format!(
        "{visibility}{is_unsafe}static {is_mut}{name}: {ty}{expr};"
    ))
}
...
pub fn variant(
    ...
    ast_from_text(&format!(
        "enum f {{ {visibility}{name}{field_list}{discriminant} }}"
    ))
}
...
pub fn struct_(
    ...
    let (semicolon, ws) = if matches!(field_list, ast::FieldList::TupleFieldList(_)) {
        (";", "")
    } else {
        ("", " ")
    };
...
pub fn enum_(
    ...
    let generic_params = generic_param_list
        .map(|it| it.to_string())
        .unwrap_or_default();
