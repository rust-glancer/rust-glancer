...
impl SyntaxFactory {
    ...
    pub fn struct_(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                strukt_name.syntax().clone(),
                ast.name().unwrap().syntax().clone(),
            );
    ...
    pub fn ty_fn_ptr<I: Iterator<Item = Param>>(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_children(
                ...
                ast.syntax()
                    .children()
                    .filter(|c| ast::Param::can_cast(c.kind())),
            );
            if let Some(ret_type) = ret_type {
                builder.map_node(
                    ret_type.syntax().clone(),
                    ast.ret_type().unwrap().syntax().clone(),
                );
            }
            builder.finish(&mut mapping);
        }
    ...
    pub fn impl_trait(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                path_type.syntax().clone(),
                ast.trait_().unwrap().syntax().clone(),
            );
    ...
    pub fn generic_ty_path_segment(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            arg_builder.map_children(
                input,
                generic_arg_list.generic_args().map(|a| a.syntax().clone()),
            );
    ...
    pub fn use_(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                use_tree.syntax().clone(),
                ast.use_tree().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn use_tree(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            if let Some(alias) = alias {
                builder.map_node(
                    alias.syntax().clone(),
                    ast.rename().unwrap().syntax().clone(),
                );
            }
            builder.finish(&mut mapping);
        }
    ...
    pub fn path_unqualified(&self, segment: ast::PathSegment) -> ast::Path {
        ...
        if let Some(mut mapping) = self.mappings() {
            let mut builder = SyntaxMappingBuilder::new(ast.syntax().clone());
            builder.map_node(
                segment.syntax().clone(),
                ast.segment().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn record_pat_field_list(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            if let Some(rest_pat) = rest_pat {
                builder.map_node(
                    rest_pat.syntax().clone(),
                    ast.rest_pat().unwrap().syntax().clone(),
                );
            }
            builder.finish(&mut mapping);
        }
    ...
    pub fn expr_call(&self, expr: ast::Expr, arg_list: ast::ArgList) -> ast::CallExpr {
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(expr.syntax().clone(), ast.expr().unwrap().syntax().clone());
            builder.map_node(
                arg_list.syntax().clone(),
                ast.arg_list().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn expr_method_call(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                receiver.syntax().clone(),
                ast.receiver().unwrap().syntax().clone(),
            );
            builder.map_node(
                method.syntax().clone(),
                ast.name_ref().unwrap().syntax().clone(),
    ...
    pub fn expr_if(
        ...
        if let Some(mut mapping) = self.mappings() {
            let mut builder = SyntaxMappingBuilder::new(ast.syntax().clone());
            builder.map_node(
                condition.syntax().clone(),
                ast.condition().unwrap().syntax().clone(),
            );
            builder.map_node(
    ...
    pub fn expr_loop(&self, body: ast::BlockExpr) -> ast::LoopExpr {
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                body.syntax().clone(),
                ast.loop_body().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn expr_for_loop(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(pat.syntax().clone(), ast.pat().unwrap().syntax().clone());
            builder.map_node(
                iterable.syntax().clone(),
                ast.iterable().unwrap().syntax().clone(),
            );
            builder.map_node(
                body.syntax().clone(),
                ast.loop_body().unwrap().syntax().clone(),
    ...
    pub fn expr_macro(&self, path: ast::Path, tt: ast::TokenTree) -> ast::MacroExpr {
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                path.syntax().clone(),
                macro_call.path().unwrap().syntax().clone(),
            );
            builder.map_node(
                tt.syntax().clone(),
                macro_call.token_tree().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn let_else_stmt(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                diverging.syntax().clone(),
                ast.let_else().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn record_expr_field(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            if let Some(ast_name_ref) = ast.name_ref() {
                ...
                let ast::Expr::PathExpr(path_expr) = ast.expr().unwrap() else {
                    unreachable!()
                };
    ...
    pub fn fn_(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                params.syntax().clone(),
                ast.param_list().unwrap().syntax().clone(),
            );
    ...
    pub fn meta_token_tree(&self, path: ast::Path, tt: ast::TokenTree) -> ast::Meta {
        ...
        if let Some(mut mapping) = self.mappings() {
            let mut builder = SyntaxMappingBuilder::new(ast.syntax().clone());
            builder.map_node(path.syntax().clone(), ast.path().unwrap().syntax().clone());
            let ast::Meta::TokenTreeMeta(meta) = &ast else {
                unreachable!()
            };
            builder.map_node(
                tt.syntax().clone(),
                meta.token_tree().unwrap().syntax().clone(),
            );
            builder.finish(&mut mapping);
        }
    ...
    pub fn impl_(
        ...
        if let Some(mut mapping) = self.mappings() {
            ...
            builder.map_node(
                path_type.syntax().clone(),
                ast.self_ty().unwrap().syntax().clone(),
            );
            if let Some(where_clause) = where_clause {
                builder.map_node(
                    where_clause.syntax().clone(),
