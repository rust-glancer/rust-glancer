...
impl ast::GenericParamList {
    ...
    pub fn find_generic_arg(&self, generic_arg: &ast::GenericArg) -> Option<GenericParam> {
        self.generic_params()
            .find_map(move |param| match (&param, &generic_arg) {
    ...
    pub fn to_generic_args(&self) -> ast::GenericArgList {
        let args = self.generic_params().filter_map(|param| match param {
            ast::GenericParam::LifetimeParam(it) => Some(ast::GenericArg::LifetimeArg(
                make::lifetime_arg(it.lifetime()?),
            )),
            ast::GenericParam::TypeParam(it) => Some(ast::GenericArg::TypeArg(make::type_arg(
                make::ext::ty_name(it.name()?),
            ))),
            ast::GenericParam::ConstParam(it) => {
                // Name-only const params get parsed as `TypeArg`s
                Some(ast::GenericArg::TypeArg(make::type_arg(
                    make::ext::ty_name(it.name()?),
                )))
            }
        });
...
impl ast::UseTree {
    ...
    pub fn split_prefix(&self, prefix: &ast::Path) {
        ...
        ted::insert(
            ted::Position::first_child_of(self.syntax()),
            prefix.syntax(),
        );
    ...
    pub fn wrap_in_tree_list(&self) -> Option<()> {
        ...
        ted::append_child(
            self.syntax(),
            make::use_tree_list(once(subtree))
                .clone_for_update()
                .syntax(),
        );
        Some(())
...
impl ast::RecordExprFieldList {
    pub fn add_field(&self, field: ast::RecordExprField) {
        ...
        ted::insert_all(
            position,
            vec![whitespace.into(), field.syntax().clone().into()],
        );
        if is_multiline {
            ted::insert(
                ted::Position::after(field.syntax()),
                ast::make::token(T![,]),
            );
        }
    }
