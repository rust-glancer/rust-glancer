...
impl WorkspaceGraphChanges {
    pub(super) fn check(
        ...
        if path.file_name() == Some(OsStr::new("Cargo.toml")) {
            ...
            if Self::is_known_graph_manifest(workspace, path) {
                return Self::Changed;
            }
            ...
            if Self::is_discoverable_workspace_member_manifest(
                workspace,
        ...
        for package in workspace.workspace_packages() {
            ...
            if autodiscovery_roots.iter().any(|root| {
                path.strip_prefix(root)
                    .is_ok_and(Self::is_auto_discovered_target_file)
            }) {
                return Self::Changed;
            }
        }

        if let Some(manifest_path) = Self::unknown_package_manifest_for_source_file(workspace, path)
            && Self::is_discoverable_workspace_member_manifest(
                workspace,
                cargo_metadata_config,
                &manifest_path,
            )
    ...
    fn unknown_package_manifest_for_source_file(
        ...
        for directory in source_path.ancestors().skip(1) {
            ...
            if manifest_path.is_file() && !Self::is_known_graph_manifest(workspace, &manifest_path)
            {
