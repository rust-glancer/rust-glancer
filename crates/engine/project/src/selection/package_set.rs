...
pub(crate) struct PhasePackageSet {
    packages: Vec<PackageSlot>,
}

impl PhasePackageSet {
    pub(crate) fn from_packages(packages: Vec<PackageSlot>) -> Self {
        Self { packages }
    }

    pub(crate) fn from_slice(packages: &[PackageSlot]) -> Self {
        Self {
            packages: packages.to_vec(),
        }
    }

    pub(crate) fn from_crates(crates: &[CrateRef]) -> Self {
    ...
    pub(crate) fn from_path(parse: &ParseDb, path: &Path) -> anyhow::Result<Self> {
        let path = path
            .canonicalize()
    ...
    pub(crate) fn as_slice(&self) -> &[PackageSlot] {
        &self.packages
    }

    pub(crate) fn into_vec(self) -> Vec<PackageSlot> {
        self.packages
    }
    ...
    pub(crate) fn iter(&self) -> impl Iterator<Item = PackageSlot> + '_ {
        self.packages.iter().copied()
    }

    pub(crate) fn filter(&self, mut predicate: impl FnMut(PackageSlot) -> bool) -> Self {
        Self {
            packages: self
                .packages
                .iter()
    ...
    pub(crate) fn package_indices(&self) -> Vec<usize> {
        self.packages.iter().map(|package| package.0).collect()
    }

    pub(crate) fn visible_dependency_subset(&self, workspace: &WorkspaceMetadata) -> PackageSubset {
