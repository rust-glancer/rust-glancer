...
impl MacroExpansionScan<'_> {
    fn start_for(self, crate_ref: CrateRef) -> usize {
        match self {
            Self::AllPending => 0,
            Self::NewCallsSince(cursors) => cursors.start_for(crate_ref),
        }
    }
}
...
/// Per-crate_ref cursors used when only macro calls generated by the previous pass should be scanned.
#[derive(Debug, Clone, Default)]
pub(crate) struct MacroExpansionCursors {
    macro_call_counts: HashMap<CrateRef, usize>,
}

impl MacroExpansionCursors {
    pub(crate) fn capture(states: &FinalizeCrateStates) -> Self {
        let mut macro_call_counts = HashMap::new();

        for package_states in states.iter_dirty() {
            for state in package_states {
                macro_call_counts.insert(state.crate_ref, state.macro_directives.len());
            }
        }
    ...
    fn start_for(&self, crate_ref: CrateRef) -> usize {
        self.macro_call_counts.get(&crate_ref).copied().unwrap_or(0)
    }
}
    ...
    for package_states in states.iter_dirty() {
        for state in package_states {
            ...
            for (call_id, directive) in state
                ...
                .iter()
                .enumerate()
                .skip(scan.start_for(state.crate_ref))
            {
...
pub(crate) fn apply_expansion_attempts(
    ...
    for attempt in attempts {
        let Some(state) = states.crate_state_mut(attempt.crate_ref) else {
            continue;
        };
        ...
        // Generated items are collected with the crate's normal name interner so later stages see
        // the same names and source references as they do for ordinary items.
        let interner = interners
            .package_mut(attempt.crate_ref.package.0)
            .with_context(|| {
                format!(
...
impl MacroExpansionAttempt {
    ...
    /// Builtin `cfg_select!` chooses one item stream using the crate's cfg environment.
    fn cfg_select(
        crate_ref: CrateRef,
        call_id: usize,
        call: &MacroCallSite,
        ...
        E: CrateResolutionEnv<Error = rg_package_store::PackageStoreError> + MacroDefinitionEnv,
    {
        ...
        let Some(path) = Path::from_macro_path_text(path_text, call.dollar_crate) else {
            return Ok(Self::skipped(state.crate_ref, call_id, call));
        };
        ...
        let resolved = match resolver.resolve(call, &path)? {
            ...
            ExpectedUnique::Ambiguous => {
                return Ok(Self::unresolved(state.crate_ref, call_id, call, path_text));
            }
        ...
        attempt.origin.dollar_crate = Some(resolved.definition.data.dollar_crate);
        Ok(attempt)
    }
    ...
    fn builtin(
        ...
        match kind {
            BuiltinMacroKind::Expr(_) | BuiltinMacroKind::IgnoredByDefMap => {
                Self::ignored_by_def_map(crate_ref, call_id, call, macro_name)
            }
