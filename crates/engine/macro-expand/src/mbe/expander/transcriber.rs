...
impl<'t> Bindings<'t> {
    ...
    fn get_fragment(
        ...
        match b {
            ...
            Binding::Nested(_) => Err(binding_err!(
                "expected simple binding, found nested binding `{name}`"
            )),
            Binding::Empty => Err(binding_err!(
                "expected simple binding, found empty binding `{name}`"
            )),
        }
    }
}
    ...
    'ops: for op in template.iter() {
        match op {
            ...
            Op::Ignore { name, id } => {
                ...
                let e = ctx
                    .bindings
                    .get_fragment(name, *id, &mut ctx.nesting, marker)
                    .err();
...
fn expand_repeat(
    ...
    loop {
        ...
        if counter == limit {
            ...
            err = Some(ExpandError::new(
                ctx.call_site,
                ExpandErrorKind::LimitExceeded,
            ));
            break;
        }
...
fn fix_up_and_push_path_tt(
    ...
    while let Some(tt) = iter.next_as_view() {
        ...
        prev_was_ident = matches!(
            tt.iter().collect_array(),
            Some([tt::TtElement::Leaf(tt::Leaf::Ident(_))])
        );
