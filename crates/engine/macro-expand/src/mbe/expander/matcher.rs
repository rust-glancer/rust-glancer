...
pub(super) fn match_<'t>(
    ...
    let mut res = match_loop(ctx_edition, pattern, input);
...
impl<'a> BindingsBuilder<'a> {
    ...
    fn push_fragment(&mut self, idx: &mut BindingsIdx, var: &Symbol, fragment: Fragment<'a>) {
        self.nodes[idx.0].push(LinkNode::Node(Rc::new(BindingKind::Fragment(
            var.clone(),
            fragment,
        ))));
    }
    ...
    fn push_missing(&mut self, idx: &mut BindingsIdx, var: &Symbol, kind: MetaVarKind) {
        self.nodes[idx.0].push(LinkNode::Node(Rc::new(BindingKind::Missing(
            var.clone(),
            kind,
        ))));
    }
        ...
        for cmd in nodes {
            match cmd {
                ...
                BindingKind::Fragment(name, fragment) => {
                    bindings
                        .inner
                        .insert(name.clone(), Binding::Fragment(fragment.clone()));
                }
        ...
        match op {
            ...
            OpDelimited::Op(Op::Var { kind, name, .. }) => {
                if let &Some(kind) = kind {
                    let mut fork = src.clone();
                    let match_res = match_meta_var(ctx_edition, kind, &mut fork, delim_span);
                    match match_res.err {
                        None => {
            ...
            OpDelimited::Op(Op::Literal(lhs)) => {
                if let Ok(rhs) = src.clone().expect_leaf() {
                    ...
                    res.add_err(ExpandError::binding_error(
                        src.clone()
                            .next()
                            .map_or(delim_span.close, |it| it.first_span()),
...
fn match_meta_var<'t>(
    ...
    let fragment = match kind {
        MetaVarKind::Path => {
            return expect_fragment(
                ctx_edition,
                input,
                parser::PrefixEntryPoint::Path,
                delim_span,
            )
            .map(Fragment::Path);
        }
    ...
    expect_fragment(ctx_edition, input, entry_point, delim_span)
...
impl<'a> OpDelimitedIter<'a> {
    fn is_eof(&self) -> bool {
        let len = self.inner.len()
            + if self.delimited.kind != tt::DelimiterKind::Invisible {
                2
            } else {
                0
            };
