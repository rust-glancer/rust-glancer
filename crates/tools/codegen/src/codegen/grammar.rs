...
use crate::{
    codegen::{
        CodegenKind, add_preamble, ensure_file_contents, grammar::ast_src::generate_kind_src,
        reformat,
    },
    project_root,
};
...
fn generate_nodes(kinds: KindsSrc, grammar: &AstSrc) -> String {
    ...
    let res = add_preamble(CodegenKind::Grammar, reformat(res));
    res.replace("#[derive", "\n#[derive")
}

fn write_doc_comment(contents: &[String], dest: &mut String) {
    for line in contents {
        writeln!(dest, "///{line}").expect("writing to String cannot fail");
    }
}
...
fn generate_syntax_kinds(grammar: KindsSrc) -> String {
        ...
        .map(|(token, name)| {
            (
                token
                    .chars()
                    .next()
                    .expect("single-byte token has one character"),
                format_ident!("{}", name),
            )
        })
        .unzip();
    ...
    let punctuation_values = grammar.punct.iter().map(|(token, _name)| {
        if "{}[]()".contains(token) {
            let c = token
                .chars()
                .next()
                .expect("bracket punctuation has one character");
            quote! { #c }
    ...
    let result = add_preamble(CodegenKind::Grammar, reformat(ast.to_string()));
        ...
        match self {
            Field::Token { token, .. } => {
                let token: proc_macro2::TokenStream =
                    token.parse().expect("field token is a valid T! token");
                Some(quote! { T![#token] })
            }
            _ => None,
        }
        ...
        let mut variant_traits = enm
            ...
            .map(|var| {
                nodes
                    .iter()
                    .find(|it| &it.name == var)
                    .expect("enum variant has a matching AST node")
            })
