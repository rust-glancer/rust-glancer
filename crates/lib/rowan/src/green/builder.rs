use crate::{
    cow_mut::CowMut,
    green::{node_cache::NodeCache, GreenArena, GreenElement, GreenNode, SyntaxKind},
    NodeOrToken,
};
use std::sync::Arc;
...
impl GreenNodeBuilder<'_> {
    ...
    pub fn token(&mut self, kind: SyntaxKind, text: &str) {
        let (hash, token) = self.cache.token(&self.arena, kind, text);
    ...
    pub fn finish_node(&mut self) {
        ...
        let (hash, node) = self
            .cache
            .node(&self.arena, kind, &mut self.children, first_child);
