...
impl MemoryReport {
    ...
    pub(crate) fn section_key(&self) -> String {
        ...
        format!("memory_{}", Self::profile_key(point))
    }
    ...
    fn from_records(
        ...
        Self {
            ...
            by_component: Self::memory_rows(Self::top_level_totals(&records), usize::MAX),
            by_kind: Self::memory_rows(Self::kind_totals(&records), usize::MAX),
            top_paths: Self::memory_rows(
                Self::string_totals(
                    records
                        .iter()
    ...
    fn top_level_totals(records: &[MemoryReportRecord]) -> Vec<(String, usize)> {
        Self::string_totals(records.iter().map(|record| {
            let path = Self::top_level_path(&record.path);
            (path, record.bytes)
        }))
    }
